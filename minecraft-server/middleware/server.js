...
require('./bootstrap-admin').bootstrap();
...
async function setupPurpurServer(instanceUuid) {
  ...
  // Every Minecraft server needs eula.txt to start at all, regardless of
  // whether the jar download succeeds or was already done in an earlier
  // session — this used to only get written on a failed download, so the
  // very first boot of a normal, successfully-downloaded server always hit
  // "You need to agree to the EULA" and had to be manually restarted once.
  ensureMinimalFiles(dataDir);
    ...
    return new Promise((resolve, reject) => {
      const get = (u, redir = 0) => {
        ...
        mod.get(u, { timeout: 120000, headers: { 'User-Agent': 'OmenHosting/1.0' } }, (r) => {
          ...
          if (r.statusCode !== 200) { ensureMinimalFiles(dataDir); return resolve(); }
...
/** Idempotent: never overwrites an existing eula.txt/server.properties, so a
 *  restart never clobbers customization the owner made through the panel. */
function ensureMinimalFiles(dataDir) {
...
async function installMinekubePlugin(instanceUuid) {
  ...
  try {
    ...
    const jarValid = fs.existsSync(jarPath) && fs.statSync(jarPath).size > 1000000;
    if (jarValid) {
    ...
    // Config/properties patches must run every call, not just on a fresh
    // install — an instance whose plugin jar already exists from a previous
    // session would otherwise never pick up a fix like the endpoint pin
    // below, since the jar-download branch above is the only place these
    // used to run.
    ensureMinekubeConfig(instanceDir, instanceUuid);
  } catch (err) {
...
/**
 * Minekube's WatchService rejects endpoint names past some length with a
 * flat "invalid endpoint name format" (400) — measured live: a 32-char
...
/** Idempotent: safe to call on every start, not just first install. */
function ensureMinekubeConfig(instanceDir, instanceUuid) {
  const endpointName = minekubeEndpointName(instanceUuid);
...
enabled: true
port: 25565
# Pinned (and kept short - see minekubeEndpointName) so the
# connect.minekube.net address is stable across restarts. Without this, the
# plugin's own default is "a random string" per its bundled config.yml — a
# fresh address every boot.
...
`);
    ...
    if (changed) {
      ...
      console.log(`[minekube] Pinned endpoint (${endpointName}) for config at ${configFile}`);
    }
  }
