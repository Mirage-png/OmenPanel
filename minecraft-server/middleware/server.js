...
const server = http.createServer((req, res) => {
  ...
  // Standalone Create Server page (no auth needed)
...
<title>OmenHosting - Create Server</title>
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  body { background:#0b0e14; color:#e0e0e0; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif; min-height:100vh; }
  .header { background:#1a2332; border-bottom:1px solid rgba(46,204,113,.15); padding:16px 24px; display:flex; align-items:center; gap:12px; }
  .header h1 { color:#2ecc71; font-size:20px; }
  .header img { height:32px; }
  .container { max-width:700px; margin:40px auto; padding:0 20px; }
  .card { background:#1a2332; border:1px solid rgba(46,204,113,.15); border-radius:12px; padding:32px; margin-bottom:24px; }
  .card h2 { color:#2ecc71; margin-bottom:8px; font-size:18px; }
  .card p { color:#82e0aa; font-size:13px; margin-bottom:20px; }
  .form-group { margin-bottom:16px; }
  .form-group label { display:block; color:#82e0aa; font-size:11px; text-transform:uppercase; letter-spacing:1px; margin-bottom:6px; }
  .form-group input { width:100%; padding:10px 12px; background:#0b0e14; border:1px solid rgba(46,204,113,.2); border-radius:6px; color:#e0e0e0; font-size:14px; outline:none; }
  .form-group input:focus { border-color:#2ecc71; }
  .btn { width:100%; padding:12px; background:linear-gradient(135deg,#2ecc71,#27ae60); border:none; border-radius:8px; color:#fff; font-size:15px; font-weight:700; cursor:pointer; transition:all .2s; }
  .btn:hover { transform:translateY(-1px); box-shadow:0 4px 20px rgba(46,204,113,.4); }
  .btn:disabled { opacity:.5; cursor:not-allowed; transform:none; }
  .status { text-align:center; margin-top:12px; font-size:13px; }
  .status.ok { color:#2ecc71; }
  .status.err { color:#e74c3c; }
  .instances { list-style:none; }
  .instances li { background:#0b0e14; border:1px solid rgba(46,204,113,.1); border-radius:8px; padding:14px 16px; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center; }
  .instances .name { color:#e0e0e0; font-weight:600; }
  .instances .addr { color:#82e0aa; font-size:12px; }
  .instances .port { color:#666; font-size:12px; }
  .sleeping { color:#f39c12 !important; }
  .btn-sm { padding:6px 14px; background:#2a2a2a; border:1px solid #444; border-radius:6px; color:#aaa; cursor:pointer; font-size:12px; text-decoration:none; display:inline-block; }
...
<body>
<div class="header">
  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#2ecc71" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 3v18M3 9h18"/></svg>
  <h1>OmenHosting</h1>
</div>
<div class="container">
  <div class="card">
    <h2>Create Minecraft Server</h2>
    <p>Instantly create a Minecraft server with Minekube Connect</p>
    <div class="form-group">
      <label>Server Name</label>
      <input id="name" type="text" placeholder="my-server" maxlength="30">
    </div>
    <button class="btn" id="createBtn" onclick="createServer()">Create Server</button>
    <div class="status" id="status"></div>
  </div>
  <div class="card">
    <h2>Your Servers</h2>
    <p id="listStatus">Loading...</p>
    <ul class="instances" id="list"></ul>
    <div class="login-link">
      <a href="/">Open Full Panel (Login)</a>
    </div>
  </div>
</div>
<script>
async function loadInstances() {
  try {
    const r = await fetch('/api/omen/instances');
    const data = await r.json();
    const list = document.getElementById('list');
    const status = document.getElementById('listStatus');
    if (!data.length) { list.innerHTML = '<div class="empty">No servers yet. Create one above!</div>'; status.textContent = ''; return; }
    status.textContent = data.length + ' server(s)';
    list.innerHTML = data.map(i => '<li>' +
      '<div><div class="name">' + esc(i.nickname) + '</div>' +
      '<div class="addr">' + (i.minekubeAddress || 'Starting...') + '</div>' +
      '<div class="port">Port: ' + i.port + (i.sleeping ? ' <span class=sleeping>Sleeping</span>' : '') + '</div></div>' +
      '<a class="btn-sm" href="/?redirect=terminal&uuid=' + i.uuid + '">Open</a></li>').join('');
  } catch(e) { document.getElementById('listStatus').textContent = 'Error loading servers'; }
}
function esc(s) { const d=document.createElement('div'); d.textContent=s; return d.innerHTML; }
async function createServer() {
  const btn = document.getElementById('createBtn');
  const st = document.getElementById('status');
  const name = document.getElementById('name').value.trim() || 'minecraft-server';
  btn.disabled = true; st.className = 'status'; st.textContent = 'Creating...';
  let userUuid = '';
  try { const w = await fetch('/api/omen/whoami'); const d = await w.json(); userUuid = d.uuid || ''; } catch {}
  try {
    const r = await fetch('/api/omen/create-server', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({nickname:name, userUuid:userUuid}) });
    const d = await r.json();
    if (d.success) {
      st.className = 'status ok'; st.textContent = 'Server created! Port ' + d.port;
      document.getElementById('name').value = '';
      loadInstances();
    } else {
      st.className = 'status err'; st.textContent = d.error || 'Failed';
    }
  } catch(e) { st.className = 'status err'; st.textContent = 'Connection error'; }
  btn.disabled = false;
}
loadInstances();
