...
enabled: true
port: 25565
# Pinned to this instance's own uuid so the connect.minekube.net address is
# stable across restarts. Without this, the plugin's own default is "a
# random string" per its bundled config.yml — a fresh address every boot.
endpoint: ${instanceUuid}
`);
    ...
    // line at all, so the plugin still falls back to its own random
    // default — patch it in retroactively.
    let cfg = fs.readFileSync(configFile, 'utf8');
    if (!/^endpoint:/m.test(cfg)) {
      cfg += `\nendpoint: ${instanceUuid}\n`;
      fs.writeFileSync(configFile, cfg);
      console.log(`[minekube] Pinned endpoint for existing config at ${configFile}`);
    }
  }
